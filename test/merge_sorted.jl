@testset "merge_sorted" begin
    @testset "Vector dedup merge" begin
        ...
        @test SA.merge_sorted([1, 3, 5], [2, 4, 6]) == [1, 2, 3, 4, 5, 6]

        # Overlapping sorted vectors
        @test SA.merge_sorted([1, 3, 5], [3, 5, 7]) == [1, 3, 5, 7]

        # Identical vectors
        @test SA.merge_sorted([1, 2, 3], [1, 2, 3]) == [1, 2, 3]

        # One empty
        @test SA.merge_sorted(Int[], [1, 2, 3]) == [1, 2, 3]
        @test SA.merge_sorted([1, 2, 3], Int[]) == [1, 2, 3]

        # Both empty
        @test SA.merge_sorted(Int[], Int[]) == Int[]

        # Single elements
        @test SA.merge_sorted([1], [2]) == [1, 2]
        @test SA.merge_sorted([2], [1]) == [1, 2]
        @test SA.merge_sorted([1], [1]) == [1]

        # Custom lt (reverse order)
        @test SA.merge_sorted([5, 3, 1], [4, 2]; lt = >) == [5, 4, 3, 2, 1]
        @test SA.merge_sorted([5, 3, 1], [5, 2]; lt = >) == [5, 3, 2, 1]

        # Mixed element types
        @test SA.merge_sorted([1, 3], [2.0, 4.0]) == [1.0, 2.0, 3.0, 4.0]
    end
    ...
    @testset "Vector combine merge" begin
        ...
        v1 = [1, 3, 5]
        v2 = [3, 5, 7]
        result = SA.merge_sorted(v1, v2, isless, +)
        @test result == [1, 6, 10, 7]

        # Filter zeros
        v1 = [1, 2, 3]
        v2 = [2, 3, 4]
        result = SA.merge_sorted(v1, v2, isless, +, x -> x < 6)
        @test result == [1, 4, 4]  # 2+2=4 kept, 3+3=6 filtered, 4 kept

        # No filter (keep all)
        result = SA.merge_sorted(v1, v2, isless, +, _ -> true)
        @test result == [1, 4, 6, 4]
    end

    @testset "In-place merge_sorted!" begin
        result = Vector{Int}(undef, 6)
        SA.merge_sorted!(result, [1, 3, 5], [2, 4, 6], isless, +)
        @test result == [1, 2, 3, 4, 5, 6]

        result = Vector{Int}(undef, 6)
        SA.merge_sorted!(result, [1, 3, 5], [3, 5, 7], isless, +)
        @test result == [1, 6, 10, 7]
    end
    ...
    @testset "Tuple dedup merge" begin
        ...
        # One empty
        @test SA.merge_sorted((), (1, 2, 3)) == (1, 2, 3)
        @test SA.merge_sorted((1, 2, 3), ()) == (1, 2, 3)
    ...
    @testset "multi_findsorted" begin
        ...
        # Custom lt (reverse order)
        @test SA.multi_findsorted([5, 3, 1], [5, 4, 3, 2, 1]; lt = >) == [1, 3, 5]
    end
