...
julia> include(joinpath(dirname(pathof(StarAlgebras)), "..", "examples", "diracs_augmented.jl"))

julia> SA.aug(p::PermutationGroups.Permutation) = 1

julia> SA.aug(f) # sum of coefficients
4

julia> SA.aug(f - f') # sum of coefficients
0
...
`b = SA.DiracBasis(G)` takes an iterator (in this case a finite
